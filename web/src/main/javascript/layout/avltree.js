...
goog.module('five.layout.AvlTree');
 ...
 * @return {T} The newly added value or the existing value if already present.
 * @override
 */
...
/**
 * Returns the existing value if the tree contains a node that equals the provided value
 *
 * @param {T} value Value to find in the tree.
 * @return {T} The existing value if found
 * @export
 */
AvlTree.prototype.find = function(value) {
  var foundValue = null;

  // Depth traverse the tree and set foundValue if we find the node
  this.traverse_(function(node) {
    var retNode = null;
    var comparison = this.comparator_(node.value, value);
    if (comparison > 0) {
